...
public/（素材）或外部目录
  → scripts/scan-assets.mjs --input <dir>（默认 D:\EngineeringManager\promo\raw）
      对每个视频跑 scene detection（默认阈值 0.3 + <1s 切点合并，实测真实录屏 0.15 过碎；ffmpeg -an scale=480:-1 select='gt(scene,0.15)',showinfo），
